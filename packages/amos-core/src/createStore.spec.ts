...
import { selectDoubleCount } from 'amos-testing';
...
describe('store.dispatch', () => {
  ...
  it('should batch dispatch', async () => {
    ...
    const [r1, r2, r3] = await Promise.all(
      store.dispatch([logout(), countBox.add(2), addTwiceAsync(2)]),
    );
    expect(r1).toBe(void 0);
    expect(r2).toEqual(2);
    expect(r3).toEqual(4);
...
describe('store.subscribe', () => {
  it('should notify listeners', async () => {
    ...
    expectCalled(fakeFn);
    const r = store.dispatch([countBox.add(1), countBox.add(2), addTwiceAsync(3)]);
    expectCalled(fakeFn);
    await Promise.all(r);
    expectCalled(fakeFn);
...
describe('select cache', () => {
  it('should not cache function', () => {
    const store = createStore();
    const selectUserFunction = selector(fn(() => ({ id: 1 })));
