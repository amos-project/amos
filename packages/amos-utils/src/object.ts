...
export interface AmosObject<T extends string> {
   ...
   * distinct factories. The `id` is different for each call of {@link createAmosObject},
   * which means hot update will change the id.
   */
  readonly id: string;
  /**
...
export function createAmosObject<T extends AmosObject<string>>(
  kind: T[typeof $amos],
  props: Omit<PartialOptional<T, 'key'>, typeof $amos | 'id'>,
): T {
